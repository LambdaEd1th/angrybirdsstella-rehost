...
impl RenderBridge {
    pub(crate) fn solve_toi_position_constraint(
        ...
        for point_index in 0..constraint.point_count() {
            ...
            let inverse_mass = (first_inverse_inertia * first_lever)
                .mul_add(first_lever, first_inverse_mass + second_inverse_mass)
                + second_inverse_inertia * second_lever * second_lever;
            if inverse_mass <= 0.0_f32 {
                continue;
            }

            // sub_1008649BC is b2ContactSolver's TOI-only position pass:
            // 0.75 Baumgarte, 0.001 slop and 0.2 maximum correction.
            let correction = (0.75_f32 * (point.separation + 0.001_f32)).clamp(-0.2_f32, 0.0_f32);
            let impulse = -correction / inverse_mass;
            ...
            if first_is_toi && let Some(object) = self.scene.get_mut(&contact.key.0) {
                object.apply_native_position_delta(
                    -first_inverse_mass * impulse_vector.0,
                    -first_inverse_mass * impulse_vector.1,
                    -first_inverse_inertia
                        * first_radius
                            .0
                            .mul_add(impulse_vector.1, -(first_radius.1 * impulse_vector.0)),
                );
            }
            if second_is_toi && let Some(object) = self.scene.get_mut(&contact.key.1) {
                object.apply_native_position_delta(
                    second_inverse_mass * impulse_vector.0,
                    second_inverse_mass * impulse_vector.1,
                    second_inverse_inertia
                        * second_radius
                            .0
                            .mul_add(impulse_vector.1, -(second_radius.1 * impulse_vector.0)),
                );
            }
        }
