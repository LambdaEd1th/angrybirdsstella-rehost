...
const PHYSICS_CAPTURE_THRESHOLD_F32: f32 = f32::from_bits(0x3D88_8889);
...
impl StellaLua {
    ...
    pub(crate) fn step_physics(&self, scaled_delta: f64) -> Result<f32, ScriptError> {
        ...
        let (step_count, uncaptured_step_count) = {
            ...
            let mut steps = 0;
            let mut uncaptured_steps = 0;
            while bridge.physics_accumulator >= PHYSICS_STEP_F32 {
                bridge.physics_accumulator += -PHYSICS_STEP_F32;
                steps += 1;
                // 0x10005EFC0 only skips the pose copy while more than two
                // fixed steps remain. Capturing each tail step retains the
                // final two consecutive solved poses on catch-up frames.
                if bridge.physics_accumulator > PHYSICS_CAPTURE_THRESHOLD_F32 {
                    uncaptured_steps += 1;
                }
            }
            (steps, uncaptured_steps)
        };
        ...
        for step_index in 0..step_count {
            ...
            // per fixed step, but it does not publish the resulting body pose
            // to Lua here. Purple waits until every catch-up step and the
            // display interpolation pass have completed (0x10005F278).
            self.render
                .lock()
                .expect("render bridge lock poisoned")
                .apply_pending_collision_velocities();
            if step_index >= uncaptured_step_count {
                self.render
                    .lock()
                    .expect("render bridge lock poisoned")
                    .capture_native_interpolation_frame();
            }
        }
